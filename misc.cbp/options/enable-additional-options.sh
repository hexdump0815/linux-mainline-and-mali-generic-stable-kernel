...
./scripts/config --set-val CONFIG_PGTABLE_LEVELS 3
...
./scripts/config --set-str CONFIG_LOCALVERSION "-stb-cbp"

for i in `cat /compile/doc/stable/misc.cbp/options/additional-options-*-yes.txt | grep -v ^#`; do
  echo $i
  ./scripts/config -e $i
done

for i in `cat /compile/doc/stable/misc.cbp/options/additional-options-*-mod.txt | grep -v ^#`; do
  echo $i
  ./scripts/config -m $i
done
